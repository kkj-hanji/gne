(function(root){
  ...
  function render(){
    ...
    if(card){card.hidden=!showExams;card.innerHTML=showExams?`<div class="exam-panel-heading"><div><p class="eyebrow">EXAM SCHEDULE · ${esc(ctx.section||"CHOOSE YOUR SECTION")}</p><h2>${title}</h2></div><span class="exam-panel-mark" aria-hidden="true">MSE</span></div>${upcoming.length?'<div class="exam-seat-status" id="today-exam-seat" role="status"><span class="exam-seat-status-dot" aria-hidden="true"></span><span>Checking your next exam seat…</span></div>':""}<div class="exam-event-list">${shown.length?shown.map(eventMarkup).join(""):`<p>${data?"No theory exam schedule is confirmed for your selected section.":"Exam information is unavailable. Check for updates or try again when connected."}`}</div>${upcoming.length?'<div class="exam-actions"><button type="button" class="primary-button" data-exam-ask="my next exam room">Find my next exam seat</button></div>':""}<p class="panel-note">${mode==="auto"?"Today returns to your timetable after your final theory exam slot ends.":"Exam view is selected. Choose Auto or Timetable in Settings to change it."}${!upcoming.length&&allTheory.length?" These published theory exams have ended.":""}</p>`:"";}
