...
const sourceUnderTest = appSource.replace(
  ...
  /function kbClean/,
  "globalThis.__brainIntegrationTest = { state, answerWithoutAi, legacyAnswerWithoutAi, runCompassBrain, setCompassBrainV2Enabled, resetBrainConversation, buildScheduleIndex, sanitizeSchedule, isHolidayCalendarQuestion };\nfunction kbClean"
);
...
test("brain 1.2 answers holiday questions across English, Hinglish, Punjabi, and Hindi", () => {
  ...
  const janamAshtami = brain.process("when is janam ashtami", context);
  assert.equal(janamAshtami.handled, true);
  assert.equal(janamAshtami.intent, "HOLIDAY_FESTIVAL_SEARCH");
  assert.equal(janamAshtami.facts.date, "2026-09-04");
  assert.match(janamAshtami.answer, /Friday, September 4, 2026/);
  assert.match(janamAshtami.answer, /Gazetted/);
  assert.doesNotMatch(janamAshtami.answer, /Second-half-day notice/);

  // Every name used by an upcoming-holiday card must round-trip through the
  // exact "when is …" question generated by the card action.
  const kernel = createBrainHarness().CompassBrainKernel;
  for (const holiday of kernel.getHolidaysForYear(2026, true)) {
    const named = brain.process(`when is ${holiday.name.toLowerCase()}`, context);
    assert.equal(named.handled, true, holiday.name);
    assert.equal(named.intent, "HOLIDAY_FESTIVAL_SEARCH", holiday.name);
    assert.equal(named.facts.date, holiday.date, holiday.name);
    assert.ok(named.answer.includes(kernel.formatIsoFull(holiday.date)), holiday.name);
  }
...
test("legacyAnswerWithoutAi answers holiday and marking scheme questions reliably", () => {
  ...
  const clickedCardQuestion = "when is janam ashtami";
  const namedHoliday = api.answerWithoutAi(clickedCardQuestion);
  assert.match(namedHoliday, /Janam Ashtami/);
  assert.match(namedHoliday, /Friday, September 4, 2026/);
  assert.equal(api.isHolidayCalendarQuestion(clickedCardQuestion), true);

  const legacyNamedHoliday = api.legacyAnswerWithoutAi(clickedCardQuestion);
  assert.match(legacyNamedHoliday, /Janam Ashtami/);
