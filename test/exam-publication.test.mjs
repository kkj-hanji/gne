...
const scripts=Object.fromEntries(await Promise.all(["exam-domain","exam-desk","admin-exams"].map(async name=>[name,await readFile(new URL(`../public/${name}.js`,import.meta.url),"utf8")])));
...
function deskHarness(){
  ...
  ctx.CompassExamDesk.setData(seed);return {ctx,document,env};
}
...
test("actual chat submit uses published seating with Roman language, date changes and profile changes",async()=>{
  ...
  for(const q of ["my exam room tomorrow","mera exam kal kis room mein hai","mera exam kal kis room me hai","mera paper kal kithe aa","2617070 exam room 25 September"]){assert.match(await ask(q),/A8 \(Automobile block\)/,q);}
